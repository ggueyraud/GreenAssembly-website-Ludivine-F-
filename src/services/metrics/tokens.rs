use sqlx::{PgPool, types::Uuid, Error};

pub async fn add(pool: &PgPool, metric_id: i32) -> Result<Uuid, Error> {
    let res = sqlx::query!("INSERT INTO metric_tokens (metric_id) VALUES ($1) RETURNING token", metric_id)
        .fetch_one(pool)
        .await?;
...
pub async fn get_metric(pool: &PgPool, token: Uuid) -> Result<i32, Error> {
    let res = sqlx::query!("SELECT metric_id FROM metric_tokens WHERE token = $1 LIMIT 1", token)
        .fetch_one(pool)
        .await?;
